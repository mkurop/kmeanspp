...
for the subsequent kmeans iterations.
'''
...
def distances_squared(points_not_choosed_thus_far: cp.ndarray,
                      center: cp.ndarray):
    """distances_squared. Computes the squared norm of differences between all points 
...
def kmeanspp(train_set: cp.ndarray, num_codevectors: int, verbose: bool = True):
    ...
    initial_codebook = cp.zeros((train_set.shape[0], num_codevectors))
...
if __name__ == "__main__":
    ...
    ts = cp.random.randn(10, 10000000)
